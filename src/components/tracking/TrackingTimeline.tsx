...
function statusIcon(status: string): string {
  ...
  if (s.includes('aguardando') || s.includes('destinatário')) return '⏳';
  if (s.includes('tentativa') || s.includes('não entregue') || s.includes('ausente')) return '⚠️';
  if (s.includes('devolvido') || s.includes('devolução')) return '↩️';
  return '•';
}
...
function statusColor(status: string, isFirst: boolean): string {
  ...
  if (s.includes('tentativa') || s.includes('não entregue') || s.includes('ausente') || s.includes('devolvido')) return 'bg-amber-400';
  return 'bg-[#C4714A]';
}

export function TrackingTimeline({ trackingCode }: { trackingCode: string }) {
  ...
  const fetch_ = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/tracking/${encodeURIComponent(trackingCode)}`);
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-3 py-4">
        ...
        <span className="text-[13px] text-[#B09C8C]">Buscando atualizações dos Correios…</span>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        {result.service && (
          <p className="text-[11px] text-[#B09C8C]">{result.service}</p>
        )}
        <button
          onClick={fetch_}
          className="ml-auto text-[11px] font-semibold text-[#C4714A] hover:text-[#A05432] transition-colors"
        >
          Atualizar
        </button>
      </div>
            ...
            <div key={i} className="flex items-start gap-4">
              ...
              <div className="flex-1 pt-1.5">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[11px] text-[#B09C8C]">{ev.location}</span>
                  <span className="text-[#E6DFD5]">·</span>
                  <span className="text-[11px] text-[#B09C8C] tabular-nums">{ev.date} às {ev.time}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      {result.source === 'cache' && (
        <p className="text-[10px] text-[#B09C8C] mt-4 text-right">
          Dados em cache · Atualizado às {new Date(result.updatedAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
        </p>
      )}
    </div>
  );
