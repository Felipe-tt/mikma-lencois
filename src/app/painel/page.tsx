...
export default function PainelDashboard() {
  ...
  useEffect(() => {
    return onSnapshot(query(collection(db,'orders'), orderBy('createdAt','desc'), limit(10)), snap => {
  ...
  return (
    <div>
      <div className="flex items-end justify-between mb-8 pb-6 border-b border-mist">
        ...
        <p className="text-xs text-faint">{new Date().toLocaleDateString('pt-BR', {weekday:'long', day:'numeric', month:'long'})}</p>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-px bg-mist mb-10">
        {kpis.map(k => (
          <div key={k.label} className={`${k.alert && k.value > 0 ? 'bg-amber-50' : 'bg-paper'} px-5 py-6`}>
            <p className={`text-2xs font-bold tracking-[0.18em] uppercase mb-3 ${k.alert && k.value > 0 ? 'text-amber-600' : 'text-faint'}`}>{k.label}</p>
            <p className={`font-display text-2xl ${k.alert && k.value > 0 ? 'text-amber-700' : 'text-ink'}`}>
